...
def test_vision_agents_runtime_uses_controlled_source() -> None:
    ...
    loaded_paths = {module_name: Path(str(importlib.import_module(module_name).__file__)).resolve() for module_name in module_names}

    for module_name, module_path in loaded_paths.items():
        assert module_path.is_relative_to(controlled_root), f"{module_name} loaded from {module_path}, expected a path under {controlled_root}"
