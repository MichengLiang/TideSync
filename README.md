...
Github Page:[08-realtime-omnimodal-call-experience-spec](https://michengliang.github.io/TideSync/books/08-realtime-omnimodal-call-experience-spec/book.html)&[09-qwen35-omni-realtime-websocket-adapter-contract](https://michengliang.github.io/TideSync/books/09-qwen35-omni-realtime-websocket-adapter-contract/book.html)

视频演示链接：https://github.com/user-attachments/assets/2363fe82-3e5a-45ba-ae3d-4916a26ed937
...
| TideSync Qwen Omni Harness 前端 | 已完成蓝图，未实现 | PR #17 只接纳前端设计和 SDK 事实材料，没有实现 React UI、后端控制 API 或 Playwright 自动化验证。 |
...
| 用户打断 AI 回答 | 已经过手动黑盒验证；adapter 契约已实现 | Batch 04 实现本地 audio flush、remote cancel、stale delta 隔离和 cancel error 保留。后续证据增强项是 `speech_started` 到 local flush / audible stop 的结构化时间戳指标。 |
...
| 错误与重连 | adapter 契约已实现并有 deterministic 证据 | Batch 06 保留结构化 Qwen error、session config failure、recoverable reconnect reset、usage parse raw payload。Qwen WebSocket 服务端重连耗时属于后续结构化观测指标。 |
...
最终符合性声明只覆盖 Batch 00-06 的 adapter contract 与本地 deterministic replay 证据；live 服务观测、打断量化延迟、重连量化时序和未文档化 payload 变体属于另一类证据面，不作为这些自动化断言的完成依据。

README 中的证据分为四类：

- 手动黑盒证据：真实浏览器、GetStream call、Vision Agents agent、Qwen Realtime、音视频链路、中文多轮、视觉问答和 barge-in 行为观察。
- Deterministic adapter 证据：fake WebSocket、event replay、pytest 和 ruff，用于证明 adapter 状态机、事件映射、工具/搜索、错误、重连和本地打断处理。
- 后续结构化指标：打断停止延迟、Qwen WebSocket 重连时序、会话 summary/export 证据产物。
- 未知项：官方未文档化 payload 变体、live function-call payload 变体和服务端瞬态行为。
...
| 符合性声明 | 记录 assertion results、测试命令、live evidence 范围、unknowns 和 rollback impact，见 [final-conformance-statement.md](forks/vision-agents-qwen-native/docs/qwen35-omni-adapter-contract/final-conformance-statement.md)。 |
...
这些命令验证的是本地 adapter contract 与 deterministic replay。项目级手动 live smoke 另见 [05 前端蓝图](docs/deep-research/05-TideSync%20Qwen%20Omni%20%E8%B0%83%E8%AF%95%E5%89%8D%E7%AB%AF%E5%AE%8C%E6%95%B4%E8%93%9D%E5%9B%BE%E6%B1%87%E6%8A%A5.md)；Batch 06 没有把 live 服务观测纳入自动化通过条件。跳过项是既有 live integration tests，后续 live Qwen service verification 需要明确的 API key、费用授权、网络和服务可用性条件。
...
| [final-conformance-statement.md](forks/vision-agents-qwen-native/docs/qwen35-omni-adapter-contract/final-conformance-statement.md) | Qwen adapter Batches 00-06 的最终断言结果、测试命令、live evidence 范围、unknowns 和 rollback impact。 |
| [UPSTREAM.adoc](forks/vision-agents-qwen-native/UPSTREAM.adoc) | 受控 Vision Agents fork 的上游来源、导入范围、删减范围和同步规则。 |

## 已知限制

- TideSync 专属 Qwen Omni Harness 前端尚未实现。当前项目已经有前端蓝图，但没有 React 控制台、后端控制 API、Playwright 自动化验证或浏览器 evidence panel。
- Qwen 原生 WebRTC transport 不属于当前实现对象。当前对象是 Qwen WebSocket adapter；浏览器到 agent 的通话媒体由 GetStream WebRTC 承担。
- Batch 06 最终符合性声明不把 live 服务观测作为 adapter contract 的自动化通过依据；项目级手动 live smoke 记录见 [05 前端蓝图](docs/deep-research/05-TideSync%20Qwen%20Omni%20%E8%B0%83%E8%AF%95%E5%89%8D%E7%AB%AF%E5%AE%8C%E6%95%B4%E8%93%9D%E5%9B%BE%E6%B1%87%E6%8A%A5.md)。
- 后续仍需结构化记录的 live 指标包括打断停止延迟、Qwen WebSocket 重连时序和未文档化 payload 变体，见 [Live Verification](forks/vision-agents-qwen-native/docs/qwen35-omni-adapter-contract/final-conformance-statement.md#live-verification)。
