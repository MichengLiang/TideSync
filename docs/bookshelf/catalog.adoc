...
|架构师与开发者
|xref:books/08-realtime-omnimodal-call-experience-spec/book.adoc#session-lifecycle[会话生命周期]；xref:books/08-realtime-omnimodal-call-experience-spec/book.adoc#observable-state-model[可观察状态模型]；xref:books/08-realtime-omnimodal-call-experience-spec/book.adoc#user-visible-feedback[用户可见反馈]
|实现对外承担的输入、输出、状态、打断、错误和限制语义。

|测试工程师与供应商评审者
|xref:books/08-realtime-omnimodal-call-experience-spec/book.adoc#part-conformance-assertions[符合性断言与不符合案例]；xref:books/08-realtime-omnimodal-call-experience-spec/book.adoc#coverage-map[覆盖映射]；xref:books/08-realtime-omnimodal-call-experience-spec/book.adoc#appendix-conformance-checklist[符合性检查表]
|给定前置状态和用户动作后，系统是否出现本书要求的可观察结果。
